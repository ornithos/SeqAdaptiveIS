...
_amis_entry(log_f, previous::Distribution, k::Int; nepochs=5, gmm_smps=1000, IS_tilt=1., terminate=0.75) = 
    amis(log_f, previous, k; nepochs=nepochs, gmm_smps=gmm_smps, IS_tilt=IS_tilt, terminate=terminate)

_amis_entry(log_f, prev::GMMComp, k::Int; nepochs=5, gmm_smps=1000, IS_tilt=1., terminate=0.75) = 
    amis(log_f, prev.pis, prev.mus, prev.covs, k; nepochs=nepochs, gmm_smps=gmm_smps, IS_tilt=IS_tilt, terminate=terminate)
    
function seq_amis(target_logfs, d::Int, k::Int; nepochs=4, IS_tilt=1.3f0, gmm_smps=2000, 
                  terminate=0.6, max_retry=3, verbose=true, min_ess=min(100, gmm_smps/2))
    init_dist = MvNormal(zeros(Float32, d), convert(Matrix{Float32}, Matrix(I, d, d)))
    seq_amis(target_logfs, init_dist, k; nepochs=nepochs, IS_tilt=IS_tilt, gmm_smps=gmm_smps, i
             terminate=terminate, max_retry=max_retry, verbose=verbose, min_ess=min_ess)
end

function seq_amis(target_logfs, init_dist::Distribution, k::Int; nepochs=4, IS_tilt=1.3f0, gmm_smps=2000, 
                  terminate=0.6, max_retry=3, verbose=true, min_ess=min(100, gmm_smps/2))
    """
         seq_amis(target_logfs, init_dist::Distribution, k::Int; nepochs=4, IS_tilt=1.3f0, gmm_smps=2000, 
                  terminate=0.6, max_retry=3, verbose=true)

    Perform AMIS on a sequence of target (unnormalized) log distributions. The initial distribution is specified
    with `init_dist` (often a standard multivariate Gaussian -- this may be specified just as an int of the
    dimension (see below), and this function will perform AMIS using the previously tuned proposal as the initial
    distribution for the next target.
    ...
    * `k`: `Int`, specifying the number of components to use for the proposal GMM.
    ...
    * A vector of the GMM components used for the final proposal for each target.
    ...
    for (i, lj) in enumerate(target_logjoints)
        ...
        for j = 1:max_retry
            S, logW, pis, mus, covs = _amis_entry(lj, c_dist, k; nepochs=nepochs, gmm_smps=gmm_smps, 
                                                     nepochs=c_nepochs, IS_tilt=c_IS_tilt, terminate=terminate)
            c_ess = eff_ss(softmax(logW))
            (c_ess > 100) && break
        ...
        verbose && printfmtln("Posterior {:d}, ess={:.2f}, time taken = {:.2f}s ({:.2f}).", t, c_ess, 
